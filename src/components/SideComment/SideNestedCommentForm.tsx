...
interface SideNestedCommentFormProps {
  sideId: number;
  commentId: number;
}

const SideNestedCommentForm = ({
  sideId,
  commentId,
}: SideNestedCommentFormProps) => {
  const commentRef = useRef({} as ParentRef);
  const { data: isAuth } = useAuth();
  const { data: userData } = useGetUser(isAuth ?? false);
  const creatCommentMutation = useCreateSideNestedComment(sideId, commentId);
  ...
  const submitComment = (event: React.FormEvent) => {
    ...
    creatCommentMutation.mutate(content, {
      onSuccess: () => {
        queryClient.invalidateQueries(getNestedCommentUrl(sideId, commentId));
...
export default SideNestedCommentForm;
